...
app.get('/api/trends', async (req, res) => {
  try {
    if (cache.data && cache.lastUpdated) {
      ...
      if (age < 5 * 60 * 1000) {
        return res.json({ ...cache.data, lastUpdated: cache.lastUpdated, cached: true });
      }
    }
    const data = await fetchAllTrends();
    res.json({ ...data, lastUpdated: cache.lastUpdated, cached: false });
  } catch (err) {
...
// Warm the cache on first request (no await — background fetch)
...
module.exports = { app, fetchAllTrends, cache };
